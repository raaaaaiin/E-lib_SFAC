...
class Awards extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.awards', ["notices" => AwardsManager::paginate(10)]);
    }

    public function noticeStatus($id, $active_status)
    {
    $notifinfo = [];
        $notice_obj = AwardsManager::find($id);
        if ($notice_obj) {
    ...
    public function editNotice($id)
    {
        ...
        if ($notice_obj) {
            ...
            $this->noticetitle = $notice_obj->noticetitle;
            $this->selcourse = $notice_obj->course;
            $this->sel_year = $notice_obj->section;
    ...
    public function add_notice()
    {
        ...
        if ($this->mode == "create" && !isset($this->selected_id)) {
            $notice_obj = new AwardsManager();
        } else {
            if (isset($this->selected_id)) {                #
                $notice_obj = AwardsManager::find($this->selected_id);
            } else {
        ...
        $notice_obj->noticetitle = !empty($this->noticetitle) ? $this->noticetitle : "N/A";
        $notice_obj->role_id = $this->give_to_role;
        $notice_obj->course = $this->selcourse;
        $notice_obj->section = $this->sel_year;
    ...
    public function deleteNotice($datas)
    {
        ...
        if ($id) {
            $obj = AwardsManager::where("id", $id)->first();
            if ($obj) {
                $obj->delete();
                $this->reset();

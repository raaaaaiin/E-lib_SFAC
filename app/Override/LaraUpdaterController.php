...
class LaraUpdaterController extends Controller
{
    ...
    {
        $this->purchase_code = Setting::where("option_key", "purchase_code")->first();
        $this->purchase_code = $this->purchase_code ? $this->purchase_code->option_value : "00";
    ...
    */
    ...
    {
        echo "<style>body{background-color: black;color:green;font-family: monospace;font-size: 15px;padding-left: 1%;padding-top: 2%;}</style>";
        echo "<h2>" . \config('app.APP_NAME') . "</h2>";
        echo '<h4><a style="color:white;" href="' . route('dashboard') . '">' . __("common.dashboard") . '</a></h4>';
        ...
        try {
            ...
            while ($zip_item = zip_read($zipHandle)) {
                ...
                if (!is_dir(base_path() . '/' . $filename)) { //Overwrite a file with its last version
                    ...
                    if (strpos($filename, 'update.sql') !== false) {
                    ...
                    if (strpos($filename, 'upgrade.php') !== false) {
                        ...
                        if (File::exists(base_path() .DIRECTORY_SEPARATOR. $filename)) $this->backup($filename); //backup current version

                        File::put(base_path() .DIRECTORY_SEPARATOR. $filename, $contents);
                        unset($contents);
    ...
    private function download($update_name)
    {
        try {
            ...
            if (!is_file($filename_tmp)) {
                $newUpdate = file_get_contents(config('laraupdater.update_baseurl') . $this->purchase_code . '/download/' . $update_name);
    ...
    */
    public function check()
    {
        ...
        if (version_compare($lastVersionInfo['version'], $this->getCurrentVersion(), ">"))
            return response()->json($lastVersionInfo);
        return '';
    }

    private function setCurrentVersion($last)
    {
        File::put(base_path() . '/version.txt', trim($last)); //UPDATE $current_version to last version
    }

    private function getLastVersion()
    {
        $content = file_get_contents(config('laraupdater.update_baseurl') . $this->purchase_code . '/laraupdater.json');
